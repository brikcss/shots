...
async function takeShots(outputDir, config = {}) {
	...
	config.viewports.forEach((viewport) => {
		config.cases.forEach((testCase) => {
			...
			promises.push(takeShot(browser, testCase, config));
		});
	});
 ...
 */
async function takeShot(browser, testCase = {}, config = {}) {
...
function setupConfig(config = {}, validate = true) {
	return new Promise((resolve) => {
		...
		if (config.fail === 'true' || configFile.fail === 'true') config.fail = true;
		if (config.fail === 'false' || configFile.fail === 'false') config.fail = false;
...
function logResults(result = {}) {
	...
	lines.push(`${colors.bold(`${prefix} ${taskName} ${resultVerb}`)}`);
	...
	if (log.getLevel() <= 3) {
		result.tests.forEach((testCase, i) => {
			...
			const resultLines = [];
			resultLines.push(testCase.result);
			if (!testCase.success) {
				if (testCase.badPixels) {
					resultLines[0] = '  ' + resultLines[0];
					resultLines.unshift(
						`${testCase.badPixels
							.toString()
							.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} bad pixels`
					);
				}
				if (testCase.diff) resultLines.push('  ' + testCase.diff);
			}
			if (testCase.baseline) resultLines.push('  ' + testCase.baseline);
			resultLines[0] = `${testCase.success ? '✓'.bold.green : '✕'.bold.red} ${
				resultLines[0]
			}`;
			if (result.task === 'test' && i + 1 < result.tests.length) resultLines.push('');
...
function startServer(options = {}) {
	cache._server = new StaticServer(
		Object.assign(
			{
				rootPath: '.',
				port: 4000
...
function baseline(config = {}) {
	return setupConfig(config)
		.then((result) => {
			...
			config = result;
			if (config.server) startServer(config.server);
...
function test(config = {}) {
	return setupConfig(config)
		.then((result) => {
			...
			config = result;
			if (config.emptyTests) fs.emptyDirSync(config.currentDir);
			if (config.server) startServer(config.server);
