...
function setupConfig(config = {}, validate = true) {
	return new Promise((resolve) => {
		...
		if (config.exit === 'true' || configFile.exit === 'true') config.exit = true;
		if (config.exit === 'false' || configFile.exit === 'false') config.exit = false;
		...
		result.tests.forEach((testCase, i) => {
			...
			if (!testCase.success) {
				if (testCase.badPixels) {
					...
					resultLines.unshift(`${testCase.badPixels} bad pixels`);
				}
			...
			if (!result.success && i + 1 < result.tests.length) resultLines.push('');
